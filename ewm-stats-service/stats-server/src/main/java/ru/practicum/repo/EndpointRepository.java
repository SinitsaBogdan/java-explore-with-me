...
public interface EndpointRepository extends JpaRepository<EndpointHit, Long> {
    ...
    @Query("SELECT new ru.practicum.dto.ViewEndpointDto(h.app, h.uri, COUNT(h.ip)) " +
    ...
    @Query("SELECT new ru.practicum.dto.ViewEndpointDto(h.app, h.uri, COUNT(distinct h.ip)) " +
            "FROM EndpointHit AS h " +
            "WHERE h.timestamp BETWEEN (:start) AND (:end) " +
            "AND (COALESCE(:uris, null) IS NULL OR h.uri IN (:uris)) " +
            "GROUP BY h.app, h.uri " +
